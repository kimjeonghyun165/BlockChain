...
contract KIP17GooNFT is KIP17, KIP17Enumerable, KIP17Metadata, MinterRole {
    ...
    function publicMint(uint256 requestedCount) external payable {
        ...
        require(
            _mintIndexForSale <= _ogNum.add(_wlNum).add(_publicNum),
            "Exceed max"
        );
        require(
    ...
    function OGMint(
        ...
        require(
            MerkleProof.verify(_merkleProof, merkleOGRoot, leaf),
            "Invalid proof!"
        );
    ...
    function WLMint(
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(_merkleProof, merkleWLRoot, leaf),
            "Invalid proof!"
        );
    ...
    function airDropMint(
        ...
        require(
            _mintIndexForSale <=
                _ogNum.add(_wlNum).add(_publicNum).add(_teamNum),
            "Exceed max"
        );
    ...
    KIP17GooNFT
{
    ...
    ) public KIP17Metadata(name, symbol) {
